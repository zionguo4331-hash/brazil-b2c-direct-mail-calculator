...
function buildWarnings(input, directMail, localWarehouse, channel, gross_revenue_usd, net_profit_usd) {
  ...
  if (input.is_direct_mail_mode && directMail.cif_usd >= 50 * DEFAULTS.warnings.cif_redline_ratio && directMail.cif_usd <= 50) {
    warnings.push({
      code: "CIF_NEAR_50_USD_THRESHOLD",
      severity: "warning",
  ...
  const target_margin = input.target_net_margin ?? 0;
  const current_margin = gross_revenue_usd === 0 ? 0 : net_profit_usd / gross_revenue_usd;
  if (current_margin < target_margin) {
    warnings.push({
      code: "BELOW_TARGET_MARGIN",
      severity: "warning",
      message: "当前净利率低于目标净利率。"
    });
  }
